...
plot_data_q1 = plot_data %>%
  filter(rurality != 'diff')

p1 = ggplot(plot_data_q1, aes(x=division, y=est, color=rurality)) + coord_flip() + geom_line() + geom_point(position = position_dodge(width = 0.4)) +
  geom_errorbar(aes(ymin = lwr, ymax = upr), position = position_dodge(width = 0.4)) + ylab("Proportion of homes with internet access.")


ggsave('w4_p2_q1_plot.png', p1, width = 5, height = 5)
p1
```
...
plot_data_q2 = plot_data %>%
  filter(rurality == 'diff')

p2 = ggplot(plot_data_q2, aes(x=division, y=est)) + coord_flip() + geom_line() + geom_point() +
