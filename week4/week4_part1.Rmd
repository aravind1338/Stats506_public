...
author: 'Group 4: Aravind Mantravadi, Erin Cikanek, Chuwen Li'
date: "9/29/2020"
...
```{r q3}
Orange %>% group_by(age) %>% mutate(zscore = (circumference - mean(circumference))/sd(circumference))
```
...
```{r q4}
Orange %>% mutate(avg_rate=circumference/age) %>% group_by(Tree) %>% summarize(avg_rate=mean(avg_rate))
```
...
The above command results in a tibble of 7 rows and 3 columns. The rows correspond to groups of trees and columns are age, avg_rate and se.
This answers the question of which group of trees has the highest average rate of growth, and an estimate of the mean growth rate per group.